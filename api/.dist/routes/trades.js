...
const AGENT_TYPES = { 1: 'ORCHESTRATOR', 2: 'ARBITRAGE', 3: 'TREND', 4: 'LIQUIDATION' };
...
async function loadAllTrades() {
    ...
    if (cache && now - cache.ts < CACHE_TTL)
        return cache;
    const ledgerAddress = process.env.POSITION_LEDGER_ADDRESS;
    if (!ledgerAddress)
    ...
    try {
        ...
        const results = await Promise.allSettled(indices.map(i => readContract({
            address: ledgerAddress,
            abi: LEDGER_ABI,
