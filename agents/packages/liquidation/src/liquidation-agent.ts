...
export class LiquidationAgent extends BaseAgent {
  ...
  protected async execute(): Promise<void> {
    ...
    if (tradeAmount < BigInt(1000)) {
      console.log(`[Liquidation] Wallet balance too low (${formatUnits(walletBalance, 6)} USDC), skipping`)
      return
    }
    console.log(`[Liquidation] Trading ${formatUnits(tradeAmount, 6)} USDC (wallet: ${formatUnits(walletBalance, 6)} USDC)`)
